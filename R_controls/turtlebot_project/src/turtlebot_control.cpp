...
TurtlebotControl::TurtlebotControl(ros::NodeHandle nh, int ID): nh_(nh), turtlebot_ID_(ID){
    ...
    odom_info_ = nh_.subscribe("/move_base_simple/goal", 1, &TurtlebotControl::odoCallback, this);
...
void TurtlebotControl::moveTurtlebot(){
    ...
    while (ros::ok()) {
        ...
        // //if the goal is within +-20 degree cone of turtlebot then its within tolerance
        // if (angle_to_target_degrees < 20 && angle_to_target_degrees > -20){
        //     linear_vel = linear_vel + 0.05; //speed up since turtlebot is on track
        //     angular_vel = 0; // no need for rotatiion, turtlebot is on track
        // }

        //  //if the goal is outside the cone of tolerance then we need to rotate the turtlebot immediately
        // if (angle_to_target_degrees > 20 && angle_to_target_degrees < -20){
        //     linear_vel = linear_vel - 0.05; // slow down as turtlebot needs more time to roate
    ...
    // double dx = currentWayPoint.x - odo_.pose.pose.position.x;
    // double dy = 
    ...
    // while (waypoint_index < waypoints_.size() && ros::ok()){
...
double TurtlebotControl::rotationAngle(double x, double y){
    ...
    double angleDiff = fabs(angle - odoYaw);
