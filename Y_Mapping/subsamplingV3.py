...
# Initialize ROS node
rospy.init_node('subsampling_publisher')

# Publisher for subsampled coordinates
subsampling_pub = rospy.Publisher('/subsampling', String, queue_size=10)
...
coord_str = ", ".join([f"({coord[0]:.2f}, {coord[1]:.2f})" for coord in subsampled_coordinates])

# Publish subsampled coordinates
subsampling_pub.publish(coord_str)

# Save subsampled coordinates to a text file
with open('subsampled_topic.txt', 'w') as txt_file:
    txt_file.write(coord_str)
